...
class _ConfigParser:
    ...
    def save_json(self, filename):
        ...
        with open(filename, "w") as f:
            ...
            self.gui.label_info.value = (
                "Saved "
                + str(len(data["configs"]))
                + ' config(s) to "'
                + filename
                + '"'
            )
        ...
        try:
            ...
            with open(filename, "r") as f:
                ...
                for config in configs:
                    ...
                    if (
                        "config_id" not in config
                        or "tx_channels" not in config
                        or "rx_channels" not in config
                        or "optimized_switching" not in config
                    ):
                    ...
                    self.configs[config["config_id"]].optimized_switching = config[
                        "optimized_switching"
                    ]
                    self.configs[config["config_id"]].tx_channels = config[
                        "tx_channels"
                    ]
                    self.configs[config["config_id"]].rx_channels = config[
                        "rx_channels"
                    ]

                self.gui.label_info.value = (
                    "Loaded " + str(len(configs)) + ' config(s) from "' + filename + '"'
                )
...
class WulpusRxTxConfigGenGUI(widgets.VBox):
    ...
    def __init__(self):
        ...
        config_hbox = widgets.HBox(
            [
                self.dropdown_configs,
                self.check_config_enabled,
                self.check_config_optimized_switching,
            ],
        ...
        self.buttons_channels_tx = [
            widgets.Button(
                description=str(i), button_style="", layout=widgets.Layout(width="12%")
            )
            for i in range(0, MAX_CH_ID + 1)
        ]
        for tx_button in self.buttons_channels_tx:
            tx_button.on_click(self.on_button_channel_tx_click)
        buttons_tx_hbox = widgets.HBox(
            [widgets.Label(value="TX", layout=widgets.Layout(width="25px"))]
            + self.buttons_channels_tx,
            layout=widgets.Layout(
                width=GUI_WIDTH, justify_content="space-between", align_items="center"
            ),
        )
        self.buttons_channels_rx = [
            widgets.Button(
                description=str(i), button_style="", layout=widgets.Layout(width="12%")
            )
            for i in range(0, MAX_CH_ID + 1)
        ...
        buttons_rx_hbox = widgets.HBox(
            [widgets.Label(value="RX", layout=widgets.Layout(width="25px"))]
            + self.buttons_channels_rx,
            layout=widgets.Layout(
                width=GUI_WIDTH, justify_content="space-between", align_items="center"
            ),
        ...
        file_buttons_hbox = widgets.HBox(
    ...
    def update_buttons(self):
        ...
        self.check_config_enabled.value = self.configs[
            self.config_index_selected
        ].enabled
        self.check_config_optimized_switching.value = self.configs[
            self.config_index_selected
        ].optimized_switching
        for i in range(0, MAX_CH_ID + 1):
            self.buttons_channels_tx[i].button_style = (
                "success"
                if i in self.configs[self.config_index_selected].tx_channels
                else "danger"
            )
            self.buttons_channels_rx[i].button_style = (
                "success"
                if i in self.configs[self.config_index_selected].rx_channels
                else "danger"
            )
