...
__interrupt void ISR_DMA (void)
{
    ...
    DMA_clearInterrupt(DMA_CHANNEL_4);
    dmaRxIsrFlag = 1;
}
...
void usStartSPI(void)
{
    ...
    DMA_disableTransfers(DMA_CHANNEL_4);
    DMA_setDstAddress(DMA_CHANNEL_4,
                      (uint32_t) s_rx_buf_1,
                      DMA_DIRECTION_INCREMENT);
    DMA_enableTransfers(DMA_CHANNEL_4);
...
{
    DMA_disableInterrupt(DMA_CHANNEL_4);
    return;
}
...
void usDmaInit(void)
{
    ...
    param_ch_0.channelSelect = DMA_CHANNEL_3;
    ...
    param_ch_0.triggerSourceSelect = DMA_TRIGGERSOURCE_15; //UCA2TXIFG
    ...
    param_ch_1.channelSelect = DMA_CHANNEL_4;
    param_ch_1.transferModeSelect = DMA_TRANSFER_REPEATED_SINGLE;
    param_ch_1.transferSize = sizeof(s_rx_buf_1);
    param_ch_1.triggerSourceSelect = DMA_TRIGGERSOURCE_14; //UCA2RXIFG
    ...
    // Don't increment address after transfer
    DMA_setSrcAddress(DMA_CHANNEL_4,
                      (uint32_t) &UCA2RXBUF,
                      DMA_DIRECTION_UNCHANGED);
}
...
void usSpiInit(void)
{
    // Configure SPI pins
    // Select Port 5
    // Set Pin 0 to input peripheral function, SPI MOSI (SIMO)
    ...
    // Select Port 5
    // Set Pin 1 to output peripheral function, SPI MISO (SOMI)
    ...
    // Select Port 5
    // Set Pin 2 to input peripheral function, SPI CLK
    // Set Pin 3 to input peripheral module Function, SPI CS
    GPIO_setAsPeripheralModuleFunctionInputPin(
        GPIO_PORT_P5,
        GPIO_PIN2 + GPIO_PIN3,
        GPIO_SECONDARY_MODULE_FUNCTION);
    ...
    EUSCI_A_SPI_initSlave(EUSCI_A2_BASE, &param);

    // Use Slave Select
    EUSCI_A_SPI_select4PinFunctionality(EUSCI_A2_BASE, EUSCI_A_SPI_ENABLE_SIGNAL_FOR_4WIRE_SLAVE);
